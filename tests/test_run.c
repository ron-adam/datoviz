...
int test_run_offscreen(TestContext* tc)
{
    DvzApp* app = tc->app;
    DvzGpu* gpu = dvz_gpu_best(app);
...
//     dvz_cmd_begin_renderpass(cmds, idx, &canvas->render.renderpass,
//     &canvas->render.framebuffers); dvz_cmd_viewport(cmds, idx, canvas->viewport.viewport);
...
//     // Run.
//     dvz_event_callback(canvas, DVZ_EVENT_REFILL, 0, DVZ_EVENT_MODE_SYNC, triangle_refill,
//     &visual); dvz_event_callback(canvas, DVZ_EVENT_FRAME, 0, DVZ_EVENT_MODE_SYNC,
//     triangle_append, &visual);
